...
import { listAllTeachers, findTeacher, createTeacher, updateTeacherPassword, deleteTeacher } from '@/lib/db';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const existing = await findTeacher(firstName, lastName);
    if (existing) {
    ...
    const result = await createTeacher(firstName, lastName, passwordHash);
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    // Hash new password
    const passwordHash = await bcrypt.hash(password, 10);
    await updateTeacherPassword(teacherId, passwordHash);
